...
export default {
  ...
  methods: {
    ...
    getResult() {
      ...
      this.number = this.randomFrom(this.value[0], this.value[1])

      this.getResultTimer = setInterval(() => {
        this.ballitems.forEach((ball, i) => {
          if (this.number === i) {
            ball.x > 225 ? ball.x-- : ball.x
            ball.x < 225 ? ball.x++ : ball.x > 225 ? ball.x : ball.x++
            ball.y > 45 ? ball.y-- : ball.y
            ball.y < 45 ? ball.y++ : ball.y > 45 ? ball.y : ball.y++
          }
        })
      }, 1)
    ...
    isCir() {
      while (true) {
        const x = Math.random() * (2 * 140) - 140
        const y = Math.random() * (2 * 140) - 140
        if (x * x + y * y <= 140 * 140) {
          return [140 + x, 140 + y]
        }
      }
...
.content {
  ...
  // align-items: center;
  @include bg('../assets/bg-content.jpg');
